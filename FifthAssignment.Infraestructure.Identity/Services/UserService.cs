	...
	public class UserRepository : IUserRepository
	{
		...
		public async Task<UserGetResponceDto> GetUserBeneficiaryAsync(string beneficiaryId)
		{
			...
			UserGetResponceDto responce = new()
			{
				...
				Roles = _userManager.GetRolesAsync(beneficiary).Result.ToList(),
		...
		public async Task<bool> UpdateAsync(UpdateUserDto user)
		{
			...
			if (userToBeUpdate.UserName != user.UserName && _userManager.FindByNameAsync(user.UserName).Result != null) return false;

			if (userToBeUpdate.Email != user.Email && _userManager.FindByEmailAsync(user.Email).Result != null) return false;
			...
			if (!string.IsNullOrEmpty(user.Password) && user.Password != userToBeUpdate.PasswordHash)
			{
				var token = await _userManager.GeneratePasswordResetTokenAsync(userToBeUpdate);

				var passwordChange = await _userManager.ResetPasswordAsync(userToBeUpdate, token, user.Password);
