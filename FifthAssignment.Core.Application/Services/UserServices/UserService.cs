...
namespace FifthAssignment.Core.Application.Services.UserServices
{
    public class UserService : IUserService
    {
        ...
        public UserService(IUserRepository userRepository, IMapper mapper, IHttpContextAccessor httpContext, IOptions<SessionKeys> sessionKeys, IBankAccountService bankAccountService)
        {
        ...
        public async Task<Result<SaveUserModel>> UpdateAsync(SaveUserModel user)
        {
            ...
            try
            {
                ...
                if (user.Amount > 0)
                {
                    var userMain = await _bankAccountService.GetAllWithAnSpecificUserIdAsync(user.Id);

                    BankAccountModel mainAcount = userMain.Data.Where(u => u.IsMain == true).FirstOrDefault();

                    mainAcount.Amount += user.Amount;

                    await _bankAccountService.UpdateAsync(new Models.BankAccountsModels.SaveBankAccountModel
                    {
                        Id = mainAcount.Id, 
                        Amount = mainAcount.Amount,
                        UserId = user.Id,
                        IdentifierNumber = mainAcount.IdentifierNumber,
                    });
                }
