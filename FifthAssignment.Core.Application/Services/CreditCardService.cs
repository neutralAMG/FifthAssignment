...
namespace FifthAssignment.Core.Application.Services
{
	public class CreditCardService : BaseProductService<CreditCardModel, SaveCreditCardModel, CreditCard>, ICreditCardService
	{
		...
		public CreditCardService(ICreditCardRepository creditCardRepository, IMapper mapper, IHttpContextAccessor httpContext, IOptions<SessionKeys> sessionKeys) : base(creditCardRepository, mapper)
		{
		...
		public virtual async Task<Result<SaveCreditCardModel>> SaveAsync(SaveCreditCardModel entity)
		{
			entity.IdentifierNumber = GenerateProductCodeString.GenerateCode();
